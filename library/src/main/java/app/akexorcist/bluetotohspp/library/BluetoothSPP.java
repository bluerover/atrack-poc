...
public class BluetoothSPP {
    ...
    private BluetoothTimeoutListener mBluetoothTimeoutListener = null;
    ...
    //TODO make this a final variable
    ...
    public BluetoothSPP(Context context) {
        ...
        mReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                Toast.makeText(mContext, "BT change received !", Toast.LENGTH_LONG).show();

                if (BluetoothDevice.ACTION_FOUND.equals(action)) {
                    Toast.makeText(mContext, device.getName() + " Device found", Toast.LENGTH_LONG).show();
                }
                else if (BluetoothDevice.ACTION_ACL_CONNECTED.equals(action)) {
                    Toast.makeText(mContext, device.getName() + " Device is now connected", Toast.LENGTH_LONG).show();
                }
                else if (BluetoothDevice.ACTION_ACL_DISCONNECT_REQUESTED.equals(action)) {
                    Toast.makeText(mContext, device.getName() + " Device is about to disconnect", Toast.LENGTH_LONG).show();
                }
                else if (BluetoothDevice.ACTION_ACL_DISCONNECTED.equals(action)) {
                    Toast.makeText(mContext, device.getName() + " Device has disconnected", Toast.LENGTH_LONG).show();
                    //BluetoothSPP.this.stopService();
                    mHandler.obtainMessage(BluetoothState.MESSAGE_STATE_CHANGE).sendToTarget();
                    //mChatService.connectionFailed();
                }
                else if (BluetoothDevice.ACTION_BOND_STATE_CHANGED.equals(action)) {
                    Toast.makeText(mContext, device.getName() + " Device has changed bond state", Toast.LENGTH_LONG).show();
                }
            }
        };
    ...
    //No Need for this, we get timeouts from watchdog
    public interface BluetoothTimeoutListener{
    ...
    private final Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            ...
            case BluetoothState.MESSAGE_TOAST:
                ...
                break;
                case BluetoothState.STATE_TIMEOUT:
                    if(mBluetoothTimeoutListener != null){
                        mBluetoothTimeoutListener.onTimeout();
                    }
                    break;
    ...
    public void autoConnect(String keywordName) {
        if(!isAutoConnectionEnabled) {
            Log.d("autoConnect", "retrying now");
            keyword = keywordName;
